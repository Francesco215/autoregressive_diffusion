...
def train(device, local_rank=0):
    vae = AutoencoderKLLTXVideo.from_pretrained("Lightricks/LTX-Video", subfolder="vae").to("cuda")

    unet = UNet(img_resolution=8, # Match your latent resolution
                img_channels=128, # Match your latent channels
                label_dim = 4,
                model_channels=128,
                channel_mult=[1,2,4],
    ...
    dataset = CsVaeDataset(clip_size=clip_length, remote='s3://counter-strike-data/dataset_compressed_LTX/', local = f'/data/streaming_dataset/cs_vae', batch_size=micro_batch_size, shuffle=False, cache_limit = '200gb')
