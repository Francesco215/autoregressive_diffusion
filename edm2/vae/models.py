...
class ContextParallelConv3d(torch.nn.Conv3d):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        **kwargs,
        ...
        assert self.stride[0] == self.kernel_size[0] or self.stride[0] == 1

        self.context_size = kernel_size[0] - 1 if self.stride[0] == 1 else 0
    ...
    def forward(self, x: torch.Tensor):
        # Compute padding amounts. and apply it

        mode = "constant" if self.padding_mode == "zeros" else self.padding_mode
        x = F.pad(x, (0, 0, 0, 0, self.context_size, 0), mode=mode)
