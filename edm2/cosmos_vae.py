...
class CausalContinuousVideoTokenizer(nn.Module):
    def __init__(
        ...
        formulation_name = kwargs.get("formulation", ContinuousFormulation.VAE.name)
    ...
    def encode(self, x):
        ...
        sample, (mean, logvar) = self.distribution(moments)
    ...
    def forward(self, input):
        sample, mean, logvar = self.encode(input)
        reconstructions = self.decode(sample)
