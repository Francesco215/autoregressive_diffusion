...
def plot_training_dashboard(
    ...
    latents = latents[:,:total_frames//2+1]
    ...
    rho_val        = 2.0
    num_steps_val  = 32
    _, mse_steps, mse_pred_values, _ = edm_sampler_with_mse(net=precond, cache=cache, target=target, sigma_max=sigma_max_val, sigma_min=sigma_min, num_steps=num_steps_val, conditioning=conditioning, rho = rho_val, guidance = guidance)
    ...
    for _ in tqdm(range(total_frames//2)):
        ...
        x, _, _, cache= edm_sampler_with_mse(precond, cache=cache, conditioning = actions, sigma_max = 80, sigma_min=sigma_min, num_steps=16, rho=7, guidance=guidance)
