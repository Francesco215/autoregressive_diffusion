...
def collate_function(batch):
    ...
    logvars = torch.tensor(np.array([b['logvar'] for b in batch]))
    actions = torch.tensor(np.array([b['action'] for b in batch]))
...
if __name__=="__main__":
    ...
    dataset = StreamingDataset(local= 'counter_strike',batch_size=micro_batch_size)
